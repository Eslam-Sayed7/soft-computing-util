...
/**
 * Recommendation runner that uses a small Sugeno rule set and the existing fuzzification utilities.
 *
 * Assumptions (documented):
 * - Category match is treated as boolean: 1.0 if user's interests include product category, else 0.0
 * - Budget score is user's budget normalized by max user budget
 * - Rating normalized by /5 and popularity normalized by max product selling_times
 * - A compact set of single-antecedent Sugeno rules are used to compute a weighted average score
 */
public class RecommendationRunner {

    public static void main(String[] args) throws Exception {
    String prodPath = args.length > 0 ? args[0] : resolveCSV("product.csv");
    String userPath = args.length > 1 ? args[1] : resolveCSV("person.csv");
    String outDir = args.length > 2 ? args[2] : "output";

        List<ProductCSVLoader.Product> products = ProductCSVLoader.loadProducts(prodPath);
        ...
        int maxSales = products.stream().mapToInt(ProductCSVLoader.Product::getSellingTimes).max().orElse(1);
        double maxBudget = users.stream().mapToDouble(UserCSVLoader.User::getBudget).max().orElse(1.0);
        ...
        for (UserCSVLoader.User user : users) {
            ...
            for (ProductCSVLoader.Product p : products) {
                ...
                double interestScore = user.getInterests().stream().anyMatch(s -> s.equalsIgnoreCase(p.getCategory())) ? 1.0 : 0.0; // assumption
            ...
            // write output file
            File out = new File(od, String.format("user_%d_recommendations.csv", user.getUserId()));
            try (BufferedWriter bw = new BufferedWriter(new FileWriter(out))) {
                ...
                for (Map.Entry<ProductCSVLoader.Product, Double> e : scored) {
                    ...
                    bw.write(String.format("%d,\"%s\",%.4f\n", p.getProductId(), p.getName(), e.getValue()));
                }
            }
    ...
    private static String resolveCSV(String name) {
        // Try several likely locations relative to module root or repository root
        String[] candidates = new String[] {
            "src/main/java/com/example/softcomputing/fuzzy/data/" + name,
            "src/main/resources/data/" + name,
            "soft-util/src/main/java/com/example/softcomputing/fuzzy/data/" + name,
            "target/classes/com/example/softcomputing/fuzzy/data/" + name,
            "data/" + name,
            "../data/" + name
        };
        ...
        throw new IllegalStateException("Could not locate CSV '" + name + "' in any of the expected locations. Tried: " + String.join(", ", candidates));
    }
}
